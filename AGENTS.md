...
- **The update check is self-spaced, and forceable.** It runs on a 30-minute persisted gap, from its own loop plus panel opens, and announces a version once. To exercise the notification without installing an older build: `defaults write com.milosrandelovic.tokenration forceUpdateCheck -bool true`, then reopen the panel or relaunch — the flag waives both the gap and the once-per-version guard, and is consumed on use. It cannot invent an update: something newer than the running build has to be released for anything to be announced.
