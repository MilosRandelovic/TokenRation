...
func missingStateResult() -> [String: Any] {
    toolResult(
        summary: "No usage data available. TokenRation has not written a reading yet — "
            + "is the app installed and running? (It publishes to "
...
func snapshotPayload(_ state: UsageState) -> [String: Any] {
    ...
    for provider in state.providers {
        ...
        if let until = provider.rateLimitedUntil {
            entry["rateLimitedUntil"] = isoString(until)
        }
    ...
    return [
        "available": true,
        "writtenAt": isoString(state.writtenAt),
        "stateFileAgeSeconds": fileAge,
...
func humanSummary(_ state: UsageState) -> String {
    ...
    for provider in state.providers {
        ...
        let age = provider.readingAge(now: now)
            .map { " — reading \(Int($0))s old" } ?? " — no reading yet"
...
while let line = readLine(strippingNewline: true) {
    ...
    case "ping":
        if let id = request.id { respond(id: id, result: [:]) }

    default:
        // Notifications (no id) need no reply — e.g. notifications/initialized.
        if let id = request.id {
            respondError(id: id, code: -32601, message: "Unknown method: \(request.method)")
        }
    }
}
