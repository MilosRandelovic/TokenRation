    ...
    static func readRateLimits(binary: String, timeout: TimeInterval) async throws -> RateLimitsResult {
        ...
        return try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { continuation in
                exchange.start(timeout: timeout, continuation: continuation)
            }
        } onCancel: {
    ...
    private static func metrics(from result: RateLimitsResult) -> [DisplayMetric] {
        ...
        if let main = result.rateLimits {
            if let secondary = main.secondary {
                metrics.append(window(secondary, id: provider.metricID("secondary"),
                                      kind: .session, title: label(for: secondary)))
            }
            if let primary = main.primary {
                metrics.append(window(primary, id: provider.metricID("primary"),
                                      kind: .window, title: label(for: primary)))
            }
            // Only show credits when the account actually has a balance to track.
            if let credits = main.credits, credits.hasCredits == true,
               let balance = credits.balance {
                metrics.append(DisplayMetric(
                    id: provider.metricID("credits"), provider: provider,
                    title: "Credits", symbolName: provider.symbol(for: .money),
                    barText: credits.unlimited == true ? "∞" : balance,
                    valueText: credits.unlimited == true ? "Unlimited" : "\(balance) remaining",
                    fraction: nil, severity: .normal, resetsAt: nil))
            }
        }

        // Per-model buckets. The "codex" entry duplicates `rateLimits`, so skip it.
        let perModel = (result.rateLimitsByLimitId ?? [:])
            .filter { $0.key != "codex" }
            .sorted { $0.key < $1.key }
        for (limitID, bucket) in perModel {
            guard let primary = bucket.primary else { continue }
            let name = bucket.limitName ?? limitID
            metrics.append(window(primary, id: provider.metricID("model:\(limitID)"),
                                  kind: .model, title: name))
        }
        return metrics
    }

    private static func window(_ window: Bucket.Window, id: String, kind: MetricKind,
                               title: String) -> DisplayMetric {
        let percent = Int((window.usedPercent ?? 0).rounded())
        return DisplayMetric(
            id: id, provider: .codex, title: title,
            symbolName: Provider.codex.symbol(for: kind),
            barText: "\(percent)%", valueText: "\(percent)% used",
            fraction: Double(percent) / 100, severity: severity(percent: percent),
...
/// anything would park the reader forever: the deadline was never re-checked, the continuation
/// never resumed, the child was never terminated, and the provider stayed stuck "refreshing".
...
private final class CodexExchange: @unchecked Sendable {
    ...
    func start(timeout: TimeInterval,
               continuation: CheckedContinuation<CodexUsageProvider.RateLimitsResult, Error>) {
        lock.lock()
        ...
        send(#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"clientInfo":{"name":"TokenRation","title":"TokenRation","version":"1"}}}"#)
    ...
    private func consume(_ chunk: Data) {
        ...
        for line in lines {
            guard let envelope = try? JSONDecoder().decode(CodexUsageProvider.Envelope.self, from: line),
                  envelope.id == 2 else { continue }
            if let result = envelope.result {
                finish(.success(result))
            } else {
                finish(.failure(UsageError.badResponse))
            }
            return
        }
    }
