...
final class Preferences {
    ...
    /// - Parameter available: providers detected on this Mac. Pins are constrained to these, so
    ///   a Codex-only machine never ends up with an unpinnable Claude placeholder in the menu
    ///   bar (there'd be no Claude tab to unpin it from).
    init(available: [Provider] = Provider.detected,
        ...
        if shownMetricIDs != stored {
            defaults.set(shownMetricIDs, forKey: Self.key)
        }
    }
    ...
    private static func migrateIfNeeded(_ stored: [String], defaults: UserDefaults) -> [String] {
        ...
        let migrated = stored.map { id -> String in
            Provider.owning(metricID: id) != nil ? id : Provider.claude.metricID(id)
        }
    ...
    func reconcile(knownIDs: Set<String>, settled: Set<Provider>) {
        ...
        let resolved = kept.isEmpty
            ? [knownIDs.sorted().first].compactMap { $0 }
            : kept
        guard !resolved.isEmpty else { return }   // nothing valid to fall back to; keep as-is
        Log.write("dropped pins no longer present in a snapshot: "
                  + "\(Set(shownMetricIDs).subtracting(resolved).sorted())")
    ...
    func toggle(_ id: String) {
        ...
        if let index = ids.firstIndex(of: id) {
            if ids.count > 1 { ids.remove(at: index) }
        } else {
            ids.append(id)
        }
        shownMetricIDs = ids
