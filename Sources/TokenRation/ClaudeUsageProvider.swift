...
struct ClaudeUsageProvider: UsageProviding {
    ...
    func fetch() async throws -> UsageSnapshot {
        ...
        guard let payload = try? JSONDecoder().decode(UsageResponse.self, from: data) else {
...
private struct UsageResponse: Decodable {
    ...
    private static func metric(from limit: Limit) -> DisplayMetric? {
        ...
        switch limit.kind {
        case "session":
            return DisplayMetric(id: Provider.claude.metricID("session"), provider: .claude,
                                 title: "Session (5-hour)",
                                 symbolName: Provider.claude.symbol(for: .session),
                                 barText: "\(percent)%", valueText: "\(percent)% used",
                                 fraction: fraction, severity: severity, resetsAt: resetsAt)
        case "weekly_all":
            return DisplayMetric(id: Provider.claude.metricID("weekly"), provider: .claude,
                                 title: "Weekly (all models)",
                                 symbolName: Provider.claude.symbol(for: .window),
                                 barText: "\(percent)%", valueText: "\(percent)% used",
                                 fraction: fraction, severity: severity, resetsAt: resetsAt)
        case "weekly_scoped":
            let name = limit.scope?.model?.display_name ?? "Model"
            return DisplayMetric(id: Provider.claude.metricID("model:\(name)"), provider: .claude,
                                 title: "\(name) (weekly)",
                                 symbolName: Provider.claude.symbol(for: .model),
                                 barText: "\(percent)%", valueText: "\(percent)% used",
                                 fraction: fraction, severity: severity, resetsAt: resetsAt)
        default:
            return nil
    ...
    private static func metric(from spend: Spend) -> DisplayMetric? {
        ...
        if let limitMoney = spend.limit {
            let limitText = Money.string(amount(limitMoney), currency: limitMoney.currency ?? currency,
                                         fractionDigits: limitMoney.exponent ?? 2)
            valueText = "\(usedText) / \(limitText) · \(percent)%"
        }
        return DisplayMetric(id: Provider.claude.metricID("spend"), provider: .claude,
                             title: "Extra usage",
                             symbolName: Provider.claude.symbol(for: .money),
                             barText: barText, valueText: valueText,
                             fraction: spend.percent.map { $0 / 100 },
                             severity: Severity(apiValue: spend.severity), resetsAt: nil)
    }

    private static func amount(_ money: Spend.Money) -> Double {
        Double(money.amount_minor ?? 0) / pow(10, Double(money.exponent ?? 2))
    }
