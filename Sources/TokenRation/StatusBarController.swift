...
private struct PanelRoot: View {
    ...
    var body: some View {
        UsagePanelView(providers: providers, prefs: prefs, updates: updates)
...
/// panel. Using our own `NSPanel` instead of `NSPopover` is deliberate: NSPopover can't
/// reposition while open (so it bounced/landed off-centre as the resizing item drifted) and
/// its transient dismissal fought the status-item click. A panel we can move smoothly with
/// `setFrame` (no close/reopen → no bounce) and dismiss explicitly via an event monitor.
///
...
final class StatusBarController: NSObject {
    ...
    init(providers: ProvidersModel, prefs: Preferences, updates: UpdateChecker) {
        ...
        self.hostingView = NSHostingView(
            rootView: PanelRoot(providers: providers, prefs: prefs, updates: updates))
        self.panel = PanelWindow(contentRect: NSRect(x: 0, y: 0, width: 300, height: 220),
        ...
        NotificationCenter.default.addObserver(
            ...
            name: NSView.frameDidChangeNotification, object: statusItem.button)
    ...
    private func render() {
        ...
        let ids = prefs.shownMetricIDs.isEmpty
            ? [Provider.claude.metricID("session")] : prefs.shownMetricIDs
    ...
    private func startSpinner() {
        ...
        spinnerTimer = Timer.scheduledTimer(timeInterval: 0.08, target: self,
                                            selector: #selector(spinnerTick), userInfo: nil, repeats: true)
    }
    ...
    private func spinnerImage() -> NSImage {
        ...
        let text = NSAttributedString(string: Self.spinnerFrames[spinnerIndex % Self.spinnerFrames.count],
                                      attributes: [.font: NSFont.systemFont(ofSize: 13),
                                                   .foregroundColor: NSColor.black,
                                                   .paragraphStyle: paragraph])
        let image = NSImage(size: NSSize(width: width, height: height))
        image.lockFocus()
        text.draw(in: NSRect(x: 0, y: ((height - text.size().height) / 2).rounded(),
                             width: width, height: text.size().height))
        image.unlockFocus()
    ...
    private func iconImage(_ symbolName: String) -> NSImage {
        ...
        if let symbol = symbolImage(symbolName) {
            symbol.draw(in: NSRect(x: ((width - symbol.size.width) / 2).rounded(),
                                   y: ((height - symbol.size.height) / 2).rounded(),
    ...
    @objc private func statusButtonFrameChanged() {
        if panel.isVisible { layoutPanel(animated: true) }
    }
    ...
    private func showPanel() {
        ...
        // Background polling is slow by design; top up when you actually look — but only if
        // the reading is stale, so opening the panel repeatedly can't spam the endpoint.
        if let model = providers.selectedModel, model.isStale() {
            Task { await model.refresh(trigger: "panel opened") }
        }
        startMonitors()
        trackTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self,
                                          selector: #selector(trackTick), userInfo: nil, repeats: true)
    }
    ...
    private func layoutPanel(animated: Bool = false) {
        ...
        let size = NSSize(width: fitting.width > 10 ? fitting.width : 300,
                          height: fitting.height > 10 ? fitting.height : 220)
        let buttonRect = window.convertToScreen(button.convert(button.bounds, to: nil))
        let origin = NSPoint(x: (buttonRect.midX - size.width / 2).rounded(),
                             y: (buttonRect.minY - size.height - 5).rounded())
    ...
    private func compositeImage(ids: [String]) -> NSImage {
        ...
        let width = sidePad * 2
            + segments.reduce(0) { $0 + $1.size.width }
            + segmentGap * CGFloat(max(segments.count - 1, 0))
    ...
    private func line(_ string: String, size: CGFloat, weight: NSFont.Weight,
    ...
    private func menuSecondary(for metric: DisplayMetric?) -> String {
        ...
        if let resetsAt = metric.resetsAt, resetsAt.timeIntervalSinceNow > 0 {
            return ResetText.short(until: resetsAt)
        }
    ...
    private static func symbolName(for id: String) -> String {
        ...
        guard parts.count == 2, let provider = Provider(rawValue: String(parts[0])) else {
    ...
    private func tooltip(ids: [String]) -> String {
        let lines = ids.compactMap { providers.metric(id: $0) }
            .map { "\($0.provider.displayName) \($0.title): \($0.valueText)" }
