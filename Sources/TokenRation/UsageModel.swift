...
final class UsageModel {
    ...
    init(provider: any UsageProviding = ClaudeUsageProvider(),
         baseInterval: TimeInterval = UsageModel.defaultInterval,
        ...
        if let legacyDeadline, legacyDeadline > Date() {
            rateLimitedUntil = legacyDeadline
        }
        ...
        if let legacyDeadline, legacyDeadline > (storedNext ?? .distantPast) {
            defaults.set(legacyDeadline, forKey: nextAttemptKey)
        }
    ...
    private func log(_ message: String) {
        Log.write("[\(source.rawValue)] \(message)")
    }
    ...
    private var nextAttemptAt: Date? {
        ...
        set {
            if let newValue { defaults.set(newValue, forKey: nextAttemptKey) }
            else { defaults.removeObject(forKey: nextAttemptKey) }
        }
    }
    ...
    private func setRateLimited(until date: Date?) {
        ...
        if let date {
            defaults.set(date, forKey: deadlineKey)
        } else {
    ...
    private func holdOff(_ seconds: TimeInterval) {
    ...
    func isStale(olderThan age: TimeInterval = 60) -> Bool {
        Date().timeIntervalSince(snapshot.updatedAt) > age
    }
    ...
    @discardableResult
    func refresh(trigger: String) async -> TimeInterval {
        ...
        do {
            ...
            let floor = min(Self.rateLimitFloor * pow(2, Double(consecutiveFailures - 1)),
                            Self.maxRateLimitWait)
            ...
            log("HTTP 429 (retry-after \(retryAfter.map { String(Int($0)) } ?? "none"), "
            ...
            let wait = jittered(min(Self.firstBackoff * pow(2, Double(consecutiveFailures - 1)),
                                    Self.maxBackoff))
            holdOff(wait)
            log("error (failures \(consecutiveFailures)): \(error.localizedDescription) "
                      + "— backing off \(Int(wait))s")
            return wait
        }
    }
    ...
    private func startNetworkMonitor() {
        pathMonitor.pathUpdateHandler = { [weak self] path in
            ...
            Task { @MainActor [weak self] in
                ...
                self.onChange?()
                if !offline {
                    await self.refresh(trigger: "reconnect")
                }
            }
        }
