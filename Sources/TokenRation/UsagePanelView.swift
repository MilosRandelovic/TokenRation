    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack(alignment: .leading) {
                ...
                if fillWidth >= 1 {
                    Capsule().fill(color).frame(width: fillWidth)
                }
            }
        }
        .frame(height: 5)
...
struct UsagePanelView: View {
    ...
    private var providerTabs: some View {
        Picker("", selection: Binding(
            ...
            set: { providers.selected = $0 }
        )) {
            ForEach(providers.available, id: \.self) { provider in
    ...
    private var content: some View {
        if let model {
            if model.snapshot.hasData {
                ForEach(model.snapshot.metrics) { metric in
                    row(metric)
                }
                ...
                statusCard(symbol: "hourglass", title: "Rate limited",
                           message: "\(providers.selected.displayName)'s usage endpoint is "
                               + "throttling requests. TokenRation will refresh "
                               + "automatically\(retryText).")
            } else if let error = model.lastError {
                statusCard(symbol: "exclamationmark.triangle.fill", title: "Unavailable",
                           message: error)
            } else {
                loadingState
            }
    ...
    private var updateBanner: some View {
        HStack(spacing: 6) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text("Version \(updates.latestVersion ?? "") available")
                    .font(.caption.weight(.medium))
                Text("brew upgrade tokenration")
                    .font(.caption2)
                    .monospaced()
    ...
    private func row(_ metric: DisplayMetric) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            if let fraction = metric.fraction {
                MeterBar(fraction: fraction, color: tint(metric.severity))
            }
            Text(caption(metric))
                .font(.caption)
                .foregroundStyle(.secondary)
    ...
    private func pin(_ id: String) -> some View {
        ...
        .buttonStyle(.borderless)
        .help(prefs.isShown(id) ? "Hide from menu bar" : "Show in menu bar")
    }

    private var footer: some View {
        HStack {
            if let model, model.snapshot.hasData {
                Text("Updated \(model.snapshot.updatedAt, format: .dateTime.hour().minute())")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Spacer()
            ...
            .buttonStyle(.borderless)
            .disabled(model?.isRefreshing ?? true)
            .help("Refresh now")
    ...
    private func caption(_ metric: DisplayMetric) -> String {
        if let resetsAt = metric.resetsAt, resetsAt.timeIntervalSinceNow > 0 {
            return "resets in \(ResetText.short(until: resetsAt))"
        }
    ...
    private var aboutView: some View {
        VStack(spacing: 10) {
            ...
            if updates.updateAvailable {
                Link(destination: updates.releasesURL) {
                    Label("Version \(updates.latestVersion ?? "") available",
            ...
            Text("Your Claude and Codex usage in the menu bar.\n\n"
                ...
                .multilineTextAlignment(.center)
                .fixedSize(horizontal: false, vertical: true)
            Link("github.com/MilosRandelovic/TokenRation",
                 destination: URL(string: "https://github.com/MilosRandelovic/TokenRation")!)
                .font(.caption)
            Divider()
            HStack {
